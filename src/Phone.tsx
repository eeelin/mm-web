...
export function Phone({onClose}:{onClose:()=>void}) {
  ...
  useEffect(()=>{load();const events=new EventSource('/api/calls/events');events.addEventListener('call-state',load);const fallback=window.setInterval(load,5000);return()=>{events.close();window.clearInterval(fallback)}},[load]);
