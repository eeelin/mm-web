...
export function Phone({onClose}:{onClose:()=>void}) {
  const [number,setNumber]=useState(''); const [calls,setCalls]=useState<Call[]>([]); const [voiceAvailable,setVoiceAvailable]=useState<boolean|null>(null); const [loading,setLoading]=useState(false); const [error,setError]=useState(''); const [showKeys,setShowKeys]=useState(false);
  const load=useCallback(async()=>{try{const response=await fetch('/api/calls',{cache:'no-store'});const data=await response.json() as CallsResponse&{error?:string};if(!response.ok)throw new Error(data.error||'服务不可用');setCalls(data.calls);setVoiceAvailable(data.voiceAvailable);setError('')}catch(err){setError(err instanceof Error?err.message:'无法读取电话状态')}},[]);
  useEffect(()=>{load();const id=window.setInterval(load,1000);return()=>window.clearInterval(id)},[load]);
  const active=calls.find(call=>call.state!=='terminated');
  const request=async(url:string,body?:unknown)=>{setLoading(true);setError('');try{const response=await fetch(url,{method:'POST',headers:body?{'Content-Type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined});const data=response.status===204?null:await response.json();if(!response.ok)throw new Error(data?.error||'操作失败');await load()}catch(err){setError(err instanceof Error?err.message:'操作失败')}finally{setLoading(false)}};
  const dial=()=>{if(number)void request('/api/calls',{number})};
  ...
  if(active) return <CallScreen call={active} loading={loading} error={error} showKeys={showKeys} onKeys={()=>setShowKeys(value=>!value)} onTone={tone} onHangup={()=>void request(`/api/calls/${active.id}/hangup`)} onClose={onClose}/>;
